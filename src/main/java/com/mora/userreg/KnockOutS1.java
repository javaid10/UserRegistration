...
// product
        ...
        } else if (request.getParameter("Product").toString().equals("MURABAHA") && request.getParameter("voucherID").toString().equals("")) {
            ErrorCode.ERR_660039.updateResultObject(result);
        } else {
            try {
                ...
                if (JsonResponse.has("customer")) {
                    if (JsonResponse.getJSONArray("customer").length() <= 0) {
                        ...
                        JSONObject job = createApplicationStage(request.getParameter("Mobile"), customerId,
                                ...
                                inOUT, request.getParameter("voucherID"));
...
//									}
                        ...
                        if (request.getParameter("Product").equals("MURABAHA")) {
                            updateCustomerIdInVoucher(customerId, request.getParameter("voucherID"));
                        }
                    }
                } else {
                    ErrorCode.ERR_660040.updateResultObject(result);
    ...
    private void updateCustomerIdInVoucher(String customerID, String voucherID) {

        HashMap<String, Object> iMap = new HashMap();
        iMap.put("customerID", customerID);
        iMap.put("voucherID", voucherID);

        try {
            DBPServiceExecutorBuilder.builder().withServiceId("DBMoraServices").withOperationId("dbxdb_sp_update_customer_id_in_voucher").withRequestParameters(iMap).build().getResponse();
        } catch (Exception e) {
            String errorMsg = "Error in updateCustomerIdInVoucher, : " + e;
    ...
    private JSONObject createApplicationStage(String mobileNumber, String cusID, String productID, String productName,
                                              ...
                                              String nanId, String age, String inout, String voucherID) {
        ...
        if (productName.equals("MURABAHA")) {
            imap.put("voucherID", voucherID);
        }
