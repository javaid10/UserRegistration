...
//					.withServiceId("DBXDBServices").withOperationId("dbxdb_customer_get").build();
				...
				if (JsonResponseCreateCus.getJSONArray("records").length() > 0) {
					...
					try {
						if (createCoreProspect(request, response, cusId)) {
							...
							// TriggerNotification.sendMessage("Dear Customer,Your Registration has been successfully completed. For inquiry: 920033800 Thank you for choosing Mora.",request.getParameter("Mobile") );
							String msh1 = "عزيزي العميل,تم إكمال تسجيلكم بنجاح.للإستفسارات: "+"920033800"+" شكراً لاختياركم مورا.";
	...
	public boolean createCoreProspect(DataControllerRequest request, DataControllerResponse response, String cusID) {
		...
		try {
			...
			String prosRes = null;
			JSONObject jsonResponse = null;
			JSONObject jsonResponseTwo = null;String dateHij = "";
			if (request.getParameter("NationalID").startsWith("1")) {
